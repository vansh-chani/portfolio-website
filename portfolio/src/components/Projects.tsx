...
import ProjectCard, {ProjectCardMobile} from "./ui/ProjectCard";
...
export function ProjectsMobile() {
    useEffect(() => {
        const observer = new IntersectionObserver((entries) => {
            entries.forEach((entry) => {
                if (entry.isIntersecting) {
                    entry.target.classList.add("project-card-mobile-appear");
                } else {
                    entry.target.classList.remove("project-card-mobile-appear");
                }
            });
        });
        document.querySelectorAll(".project-card-mobile").forEach((el) => observer.observe(el));
        return () => observer.disconnect();
    }, []);
    return (
        <section id="projects" className="flex flex-col items-center pb-40">
            <h1 className="heading-projects font-jetBrains-mono text-3xl font-extrabold w-[90vw] md:w-[60vw] p-4 mt-10 pb-10 mb-10 text-white border-b-1 border-b-white/30 text-center">PROJECTS</h1>
            {projectList.map((project, index) => (
